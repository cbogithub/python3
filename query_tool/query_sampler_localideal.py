...
# load restaurant deal terms
(ent_deal, cum_deal, ttl_deal) = load_entities_and_weights_from_file("m4_deal_terms.txt")
len_deal = len(ent_deal)
print("loaded " + str(len_deal) + " restaurant deal terms ... ")

# process one random patterns
sample_size = 200
timestr = time.strftime("%m%d%y_%H%M%S", time.localtime())
out_file_name = "sample_queries_m4_localdeal_" + str(timestr) + ".txt"
...
with open(out_file_name, 'w', newline='', encoding='utf-8') as out_file:
    ...
    while sample_size > 0:
        ...
        deal_cnt = pattern.count('[deal]')
        query_cnt = pattern.count('[query]')
        geocity_cnt = pattern.count('[geocity]')
        #    print "query pattern: " + pattern

        while deal_cnt > 0:
            deal = random_samlple_weighted_array(ent_deal, cum_deal, ttl_deal)
            pattern = pattern.replace('[deal]', deal)
            deal_cnt = deal_cnt - 1
